class Recorder {
    ...
    _setup() {
        ...
        const options = commonCodecs.map(codec => ({ mimeType: `${this.videoType};${codec}` }))
            .find(options => MediaRecorder.isTypeSupported(options.mimeType))
        if (!options) {
            throw new Error(`nonce of the codecs:${commonCodecs.join(',')} are supported`)
    ...
    startRecording() {
        ...
        console.log(`Media Recorted started`,this.mediaRecorder)
